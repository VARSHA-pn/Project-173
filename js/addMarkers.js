  ...
  init: async function() {
    ...
    toys.map(toy => {
      ...
      if (!toy.is_out_of_stock) {
        ...
        model.setAttribute("position", toy.modal_geometry.position);
        model.setAttribute("rotation", toy.modal_geometry.rotation);
        model.setAttribute("scale", toy.modal_geometry.scale);
        model.setAttribute("gltf-model", `url(${toy.modal_url})`);
